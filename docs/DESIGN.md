...
- Pluggable storage with three backends: `postgres`, `sqlite`, `file`.
  PostgreSQL is the production backend for paste.example.com; sqlite and file
  exist for low-dependency self-hosting.
- Single static binary, no CGO, deployable on a `distroless`/`scratch` base.
- Configured entirely through the `STORAGE_*` environment contract (section 5).
...
|------------|------------------------|-------------------------------------------------------|
| POST       | `/api/pastes`          | Create a paste. Body is raw text (`text/plain`), or multipart field `content`. |
| GET/HEAD   | `/api/pastes/{id}`     | Return JSON `{"id": "...", "content": "..."}`.       |
| GET/HEAD   | `/api/pastes/{id}/raw` | Return the raw paste body as `text/plain`.           |
...
Admin routes (section 8) are unchanged in shape: `/admin`, `/admin/login`,
`/admin/callback`, `/admin/logout`, `/admin/api/pastes`,
`DELETE /admin/api/pastes/{key}`, `POST /admin/api/purge`.

### 3.2 Response shapes and status codes

Every error response, public and admin, has the shape `{"error": "<message>"}`.
...
- 200 `application/json`: `{"id": "<key>", "content": "<text>"}`
- 404 `application/json`: `{"error": "No paste exists for that id."}`
...
Any request may also fail with:

- 500: `{"error": "The storage backend is unavailable."}` (backend unreachable)
...
only `res.id` (POST) and `res.content` (GET); the error messages are kept for
API consumers.
...
- `phonetic` mode assembles a key from randomly chosen syllable templates
  (`cv`, `cvc`, `vc`, `cvv`, `ccv`), filling each slot with a `crypto/rand`-picked
  consonant or vowel, concatenating the syllables, and truncating the result to
...
Uses a `pastes` table, created automatically on first connect via
...
--   $3 = now() + expire interval when expiry configured, else NULL

-- get
SELECT id, body, expires_at FROM pastes
  WHERE slug = $1 AND (expires_at IS NULL OR expires_at > now());
--   expired rows are filtered at read, not deleted

-- bump expiry on read (when configured and not skipped)
UPDATE pastes SET expires_at = $1 WHERE id = $2;
```
...
schema and expiration semantics as postgres, using SQLite native types:
...
create table pastes (
  ...
  expires_at integer null       -- unix seconds; NULL = never
);
```
...
One file per paste under a base directory, filename = the sha256 of the key,
hex-encoded (prevents path traversal via key content; the hash is a filename
derivation, not a security primitive). The file backend has no expiration.
...
- **Built-in themes.** Each alternate is a file under `web/static/themes`
  (`arctic`, `dark`, `umbra`, `ember`, `moss`), embedded in the
  binary.
...
gopaste ships as a single static container image (`ghcr.io/rake-pro/gopaste`)
and runs behind any reverse proxy; all configuration is via environment
variables (section 5). Deployment specifics (cluster, chart, secrets
management) live in the operator's own infrastructure repo, not in this one.
