...
- **BREAKING:** Public paste API redesigned under `/api/pastes`: `POST
  ...
  raw body. Replaces the previous paste-creation and raw-read routes. Error
  responses are now a consistent `{"error":"<message>"}` shape across the
  public and admin APIs.
- **BREAKING:** Storage schema replaced. Both the postgres and sqlite backends
  now use a `pastes` table (`slug`, `body`, `created_at`, `expires_at`),
  auto-created on startup; the previous table is no longer read.
- File backend filenames are now a sha256 hex digest of the key (previously a
  different digest algorithm).
- Phonetic key generator reimplemented around a syllable-template design:
  keys are assembled from randomly chosen templates (`cv`, `cvc`, `vc`, `cvv`,
  `ccv`) with each slot filled by `crypto/rand`.
- Docs (README, CLAUDE.md, docs/DESIGN.md, docs/AUTH.md, web/about.md) updated
  to match the new API contract and schema.
...
- Schema: added a nullable created timestamp column to the storage schema
  (additive, idempotent `ADD COLUMN IF NOT EXISTS` on postgres / probed on
  sqlite). Pre-existing rows keep NULL created ("unknown" in the admin
...
- Hardened ahead of public launch (post-review):
  - Reject empty/whitespace-only pastes server-side (paste creation -> 400).
  ...
  - CSRF: paste creation blocks cross-site browser requests via
...
- Theming overhaul. Themes are now standalone CSS files (one `[data-theme]`
  ...
  - New built-in themes: generic `dark`, plus three distinct dark palettes
    `umbra` (plum/amber), `ember` (copper/olive), and `moss` (forest/amber)
...
- HTTP + JSON API: paste creation (raw body or multipart field), paste read,
  paste raw-read, and a frontend SPA route. Extension-stripping on keys,
...
- Storage backends (all pure-Go / CGO-free, all compiled in): `postgres`
  ...
  (one file per paste, hash-of-key filename).
